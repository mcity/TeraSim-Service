...
def test_should_pause_and_resume_simulation_successfully(simulation_instance):
    ...
    pause_response = requests.post(
        f"{BASE_URL}/simulation_control/{simulation_instance}", json={"command": "pause"}
    )
    assert pause_response.status_code == 200

    # Resume the simulation
    resume_response = requests.post(
        f"{BASE_URL}/simulation_control/{simulation_instance}", json={"command": "resume"}
    )
...
def test_should_handle_different_auto_run_modes(auto_run_enabled):
    ...
    start_response = requests.post(f"{BASE_URL}/start_simulation", json=simulation_config)
